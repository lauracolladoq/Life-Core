        ...
        <div class="feeds">
            @foreach ($posts as $post)
                <div class="feed">
                            ...
                            @if ($post->user->id == auth()->user()->id)
                                <div class="user">
                                    <div class="profile-picture">
                                        <img src="{{ Storage::url($post->user->avatar) }}" alt="{{ $post->user->username }}'s profile picture" />
                                    </div>
                                    <div class="info">
                    ...
                    <div class="feed-img">
                        <img src="{{ Storage::url($post->image) }}" alt="Image from {{ $post->user->username }}" />
                    </div>
                    ...
                    @elseif ($post->usersLikes->count() == 1)
                        <div class="liked-by">
                            <span><img src="{{ Storage::url($post->usersLikes()->inRandomOrder()->value('avatar')) }}"
                                    alt="Avatar of {{ $post->usersLikes()->inRandomOrder()->value('username') }}" /></span>
                    ...
                    @else
                        ...
                        <div class="liked-by">
                            <span><img src="{{ Storage::url($post->usersLikes()->inRandomOrder()->value('avatar')) }}"
                                    alt="Avatar of {{ $post->usersLikes()->inRandomOrder()->value('username') }}" /></span>
                            <span><img src="{{ Storage::url($post->usersLikes()->inRandomOrder()->value('avatar')) }}"
                                    alt="Avatar of another user who liked this post" /></span>
                                ...
                                @auth
                                    @if ($comment->user_id == auth()->user()->id)
                                        <div class="comment-user">
                                            <div class="profile-picture">
                                                <img src="{{ Storage::url($comment->user->avatar) }}" alt="{{ $comment->user->username }}'s profile picture" />
                                            </div>
                                    ...
                                    @else
                                        <div class="comment">
                                            ...
                                            <div class="comment-body">
                                                <a href="{{ route('user-profile', $comment->user->id) }}" class="font-extrabold">{{ $comment->user->username }}</a>
                                                <p>{{ $comment->content }}</p>
                                            </div>
                                        </div>
                                    @endif
